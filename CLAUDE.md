...
- localStorage ใหม่ **`atlas-quiz-v1`** `{ <tid>: { <ลำดับ quiz2 ในหัวข้อ>: {done, ok, n, t} } }` — quiz2 (`_work/std2/js/quiz.js`) ส่งเหตุการณ์ `std2:quiz` ทุกครั้งที่ตอบ ห้ามตั้งชื่อคีย์ชนกับ `atlas-sula-v1` `atlas-bm-v1` `atlas-last-v1` `atlas-sem-v1` `atlas-mode-v1` `atlas-rail-v1`
